...
## 🔔 初回起動時の注意（重要）
- Trivago 等の外部サイトのクッキーバナーが表示される場合は「OK / 同意」を手動で押してください（自動で押せないケースあり）。
- Google の自動サインイン（ブラウザの「他のアカウントで自動サインイン」等）はオンのままだと自動遷移時に望ましくないポップアップやログイン切替が発生するため、可能であれば「オフ」にしておくとスムーズに動作します。
